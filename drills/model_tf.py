...
class A2C:
    def __init__(self, options, load_model=False, fpga_mapping=False):
        ...
        self.writer = tf.summary.FileWriter("./tensorboard_logs", self.session.graph)
        self.summary_loss = tf.summary.scalar("Loss", self.loss_val)
        self.cumulative_reward = tf.placeholder(tf.float32, shape=None, name='cumulative_reward')
        self.summary_reward = tf.summary.scalar("Cumulative Reward", self.cumulative_reward)
        

        self.training_time_placeholder = tf.placeholder(tf.float32, shape=None, name='training_time')
        self.summary_training_time = tf.summary.scalar("Training Time", self.training_time_placeholder)
    ...
    def train_episode(self):
        ...
        feed_dict = {
            ...
            self.cumulative_reward: np.sum(episode_rewards)  # Total reward for the episode
        }
        
        _, summary_loss, summary_reward = self.session.run(
            [self.train_op, self.summary_loss, self.summary_reward],
            feed_dict=feed_dict
        )
        
        # Calculate and log training time
        end = time.time()
        training_time = end - start  # Training time in seconds
        feed_dict[self.training_time_placeholder] = training_time
        summary_training_time = self.session.run(self.summary_training_time, feed_dict={self.training_time_placeholder: training_time})
        
        self.writer.add_summary(summary_loss, global_step=self.game.iteration)
        self.writer.add_summary(summary_reward, global_step=self.game.iteration)
        self.writer.add_summary(summary_training_time, global_step=self.game.iteration)
        self.writer.flush()
...
class A2C:
    ...
    def train_episode(self):
        ...
        advantage = discounted_episode_rewards - state_values.squeeze()
